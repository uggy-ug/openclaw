...
- Long replies: tap **View all** on a capped assistant reply to load its complete text. The **Full text** reader displays plain text, including Markdown source, while the formatted preview and attachments stay in the conversation. Very long answers use numbered pages with first, previous, next, and last-page controls to keep memory use bounded without dropping text. Android's text-selection controls apply to the displayed page. Close returns to the preview and its message actions; loading failures show an explicit retry, reconnect, or Gateway-update outcome.
- Send: `chat.send`
