...
suite.define(() => {
  it.each([
    ...
    { reducedMotion: "no-preference", interruption: "pointer", recoveryPosition: "above-viewport" },
  ] as const)(
      ...
      await suite.withPage(
        ...
        async ({ page }) => {
          ...
          if (interruption === "wheel") {
            ...
            // Interrupt on the first native scroll event reaching this case's geometry:
            // Node round trips can outlive the row's range. Pointer intent is synthetic;
            // smooth animation and its cancellation still use the real browser.
            during = await page.locator(".chat-scroll-to-bottom").evaluate(
              (button, { waitTimeout, recoveryPosition: requestedPosition }) => {
                const scroller = document.querySelector<HTMLElement>(
                  ".chat-pane-cache__pane--active .chat-thread",
                )!;
                return new Promise<{ top: number; max: number }>((resolve, reject) => {
                  const interrupt = (event: Event) => {
                    ...
                    if (requestedPosition === "above-viewport") {
                      const row = scroller
                        .querySelector<HTMLElement>(
                          '.chat-bubble[data-entry-id="sizing-message-1"]',
                        )
                        ?.closest<HTMLElement>(".chat-virtual-row");
                      if (
                        !row ||
                        row.getBoundingClientRect().bottom > scroller.getBoundingClientRect().top
                      ) {
                        return;
                      }
                    }
              ...
              { waitTimeout: controlUiE2eWaitTimeoutMs, recoveryPosition },
            );
          ...
          const interruptedRow = await bubble.evaluate((element) => {
            const row = element.closest<HTMLElement>(".chat-virtual-row")!;
            return {
          ...
          expect(interruptedRow.key).toBe(initial.key);
          if (interruption === "pointer") {
            expect(
              interruptedRow.bottom <= interruptedRow.viewportTop,
